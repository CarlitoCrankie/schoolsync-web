...
useEffect(() => {
  // Only set up activity listeners if onLogout is provided
  if (typeof onLogout === 'function') {
  ...
  // Determine user role and capabilities - FIXED
  const isCompanyAdmin = user?.role === 'company_admin' || user?.role === 'main_admin'
  const isSchoolAdmin = user?.role === 'school_admin' || user?.SchoolID
  ...
  // FIXED: Added Settings tab for school admins
  const availableTabs = isCompanyAdmin ? [
    { id: 'dashboard', label: 'Dashboard', icon: '📊' },
    { id: 'attendance', label: 'Network', icon: '👥' },
    { id: 'schools', label: 'Schools', icon: '🏫' },
  ...
  const loadCompanyAdminData = async () => {
    try {
      ...
      // Enhanced stats calculation with sync data
      if (overviewData.success && overviewData.overview) {
  ...
  // const loadSchoolAdminData = async () => {
  //   try {
  //     console.log('=== DEBUGGING SCHOOL ADMIN DATA LOADING ===')
  //     console.log('User object:', user)
  //     console.log('User school_id:', user.SchoolID)
  //     console.log('User role:', user.role)

  //     const schoolId = user.SchoolID || user.school_id || 2
  //     console.log('Final schoolId being used:', schoolId)

  //     // Test each API endpoint individually
  //     console.log('Making API calls...')
      
  //     const overviewUrl = `/api/analytics?type=overview&school_id=${schoolId}`
  //     const studentsUrl = `/api/students?school_id=${schoolId}&include_stats=true`
  //     const attendanceUrl = `/api/analytics?type=real-time&school_id=${schoolId}`
      
  //     console.log('Overview URL:', overviewUrl)
  //     console.log('Students URL:', studentsUrl)
  //     console.log('Attendance URL:', attendanceUrl)

  //     const [overviewResponse, studentsResponse, attendanceResponse, syncStatusResponse, settingsResponse] = await Promise.all([
  ...
  //       fetch(`/api/sync-agent?school_id=${schoolId}`, {
  //         method: 'POST',
  //         headers: { 'Content-Type': 'application/json' },
  //         body: JSON.stringify({ action: 'get_status', school_id: schoolId })
  //       }),
  //       fetch(`/api/school-settings?school_id=${schoolId}`).catch(() => ({ json: () => ({ success: false }) }))
  //     ])
  ...
  //     // Store school time settings
  //     if (settingsData.success && settingsData.settings) {
  //       setSchoolTimeSettings(settingsData.settings)
  //     }

  //     // FIXED: Determine sync status with school-specific filtering
  //     let syncAgentStatus = 'offline'

  //     console.log('Sync Status Response for School', schoolId, ':', syncStatus)

  //     if (syncStatus.success) {
  //       // Check if this is school-specific sync status
  //       if (syncStatus.school_id && syncStatus.school_id == schoolId) {
  //         syncAgentStatus = syncStatus.status || 'offline'
  //       } else if (syncStatus.agents && syncStatus.agents.length > 0) {
  //         // From sync-performance endpoint - find this school's agent
  //         const schoolAgent = syncStatus.agents.find(agent => agent.school_id == schoolId)
  //         if (schoolAgent) {
  ...
  //         // From sync agent ping response - make sure it's for this school
  //         syncAgentStatus = 'online'
  //       } else {
  //         // Fallback: assume offlisne if no school-specific data
  ...
  //     console.log('Final sync agent status:', syncAgentStatus)

  //     // Enhanced stats with sync status - ENSURE SCHOOL-SPECIFIC DATA
  //     if (overviewData.success && overviewData.overview && overviewData.overview.school_id == schoolId) {
  //       setStats({
  //         total_students: overviewData.overview.students?.total || 0,
  //         present_today: overviewData.overview.attendance?.today || 0,
  //         absent_today: Math.max(0, (overviewData.overview.students?.total || 0) - (overviewData.overview.attendance?.today || 0)),
  ...
  //       })
  //     } else {
  //       // Always use school-specific fallback calculations
  //       const totalStudents = studentsData.success ? studentsData.data?.length || 0 : 0
  //       const presentToday = attendanceData.success ? 
  //         (attendanceData.current_activity?.filter(r => r.status === 'IN' && r.school_id == schoolId).length || 0) : 0
  ...
  //         students_without_passwords: 0,
  //         sync_status: syncAgentStatus || 'offline'
  //       })
  //     }
  ...
  //     // Enhanced attendance loading with recent data
  //     if (attendanceData.success) {
  //       if (attendanceData.current_activity) {
  //         // Get recent check-ins (last 24 hours)
  //         const recentCheckIns = attendanceData.current_activity
  //           .filter(record => {
  //             const recordDate = new Date(record.scan_time || record.created_at)
...
const loadSchoolAdminData = async () => {
  ...
  try {
    ...
    // FIXED API CALLS
    const [overviewResponse, studentsResponse, attendanceResponse, syncStatusResponse] = await Promise.all([
      fetch(`/api/analytics?type=overview&school_id=${schoolId}`),
      fetch(`/api/students?school_id=${schoolId}&include_stats=true`),
      fetch(`/api/analytics?type=real-time&school_id=${schoolId}`),
      fetch(`/api/sync-agent`, {  // REMOVED school_id from URL
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: 'check_database_structure' })  // Use existing action
      })
      // REMOVED school-settings call since API doesn't exist
    ])
    ...
    console.log('Sync Response Status:', syncStatusResponse.status)
    console.log('Sync Status Data:', syncStatus)

    // Check what school_id is in the data
    if (overviewData.success) {
      console.log('Overview contains school_id:', overviewData.overview?.school_id)
      console.log('Does it match our schoolId?', overviewData.overview?.school_id == schoolId)
    }

    if (attendanceData.success && attendanceData.current_activity) {
      console.log('Attendance records school IDs:', 
        attendanceData.current_activity.map(r => r.school_id).filter((v, i, a) => a.indexOf(v) === i)
      )
      console.log('Total attendance records:', attendanceData.current_activity.length)
      console.log('Records for our school:', 
        attendanceData.current_activity.filter(r => r.school_id == schoolId).length
      )
    }
    console.log('=== END API RESPONSE DEBUGGING ===')

    // FIXED: Simplified sync status - just check if database is working
    let syncAgentStatus = syncStatus.success ? 'online' : 'offline'
    console.log('Final sync agent status:', syncAgentStatus)

    // ALWAYS use fallback calculations since APIs might not be school-specific
    const totalStudents = studentsData.success ? studentsData.data?.length || 0 : 0
    const presentToday = attendanceData.success ? 
      (attendanceData.current_activity?.filter(r => r.status === 'IN' && r.school_id == schoolId).length || 0) : 0
    ...
    console.log('Present today filtered by school:', presentToday)
    ...
    if (studentsData.success && studentsData.data) {
      console.log('Setting students data from API, count:', studentsData.data.length)
      setStudents(studentsData.data.map(student => ({
        id: student.student_id,
        name: student.name,
        grade: student.grade,
    ...
    // Handle attendance data - FILTER BY SCHOOL
    if (attendanceData.success && attendanceData.current_activity) {
      ...
      const recentCheckIns = attendanceData.current_activity
        .filter(record => {
          ...
          return isRecent && isOurSchool
        })
        .sort((a, b) => new Date(b.scan_time || b.created_at) - new Date(a.scan_time || a.created_at))
        .slice(0, 10)

      console.log('Filtered recent check-ins for our school:', recentCheckIns.length)
      setAttendance(recentCheckIns.map(record => ({
        id: record.attendance_id,
        studentName: record.student_name,
        status: record.status,
    ...
    console.log('=== END DEBUGGING SCHOOL ADMIN DATA LOADING ===')
  ...
  const loadFallbackSchoolData = async () => {
    try {
      ...
      setStats({
        ...
        absent_today: 0,
        students_without_passwords: 0,
        sync_status: statsData.success ? 'online' : 'offline'
      })

      if (attendanceData.success) {
        setAttendance(attendanceData.result.recentRecords?.slice(0, 10).map(record => ({
          id: record.AttendanceID,
          studentName: record.StudentName,
          status: record.Status,
          time: record.ScanTime,
  ...
  const determineSystemHealth = (overview, syncData) => {
    ...
    // If no agents configured, assume development/single-school setup
    if (totalAgents === 0) {
      return 'no_agents'
    }

    // If no agents are online, it's definitely an error
    if (onlineAgents === 0) {
      return 'error'
    }
    ...
    // More reasonable health logic
    if (onlinePercentage >= 60) {
      return 'healthy'    // 60%+ agents online (3+ out of 5)
    } else if (onlinePercentage >= 20) {
      return 'degraded'   // Some agents online (1-2 out of 5)
    } else {
      return 'error'      // Less than 20% online
    }
  }
    ...
    <div className="max-w-2xl">
      ...
      <form onSubmit={handleSaveSettings} className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                required
              />
              <p className="text-xs text-gray-500 mt-1">When does school officially start?</p>
            </div>
            ...
            <div>
              ...
              <input
                ...
                required
              />
              <p className="text-xs text-gray-500 mt-1">When does school officially end?</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-gray-50 p-6 rounded-lg">
          <h4 className="text-md font-medium text-gray-900 mb-4">Attendance Timeline</h4>
          <div className="space-y-3">
            <div className="flex items-center">
              <div className="w-4 h-4 bg-green-400 rounded-full mr-3"></div>
              <span className="text-sm">
                <strong>On Time:</strong> Check-in before {settings.late_arrival_time}
              </span>
            </div>
            <div className="flex items-center">
              <div className="w-4 h-4 bg-red-400 rounded-full mr-3"></div>
              <span className="text-sm">
                <strong>Late Arrival:</strong> Check-in after {settings.late_arrival_time}
              </span>
            </div>
            <div className="flex items-center">
              <div className="w-4 h-4 bg-blue-400 rounded-full mr-3"></div>
              <span className="text-sm">
                <strong>Normal Departure:</strong> Check-out after {settings.early_departure_time}
              </span>
            </div>
            <div className="flex items-center">
              <div className="w-4 h-4 bg-orange-400 rounded-full mr-3"></div>
              <span className="text-sm">
                <strong>Early Departure:</strong> Check-out before {settings.early_departure_time}
              </span>
            </div>
          </div>
        </div>
      ...
      <div className="mt-8 bg-blue-50 border border-blue-200 rounded-lg p-4">
        ...
        <ul className="text-xs text-blue-800 space-y-1">
          <li>• Students checking in before the Late Arrival time will be marked as "On Time"</li>
          <li>• Students checking in after the Late Arrival time will be marked as "Late Arrival"</li>
          <li>• Students checking out before the Early Departure time will be marked as "Early Departure"</li>
          <li>• Students checking out after the Early Departure time will be marked as normal "Check Out"</li>
          <li>• These settings apply to all future attendance records for your school</li>
        </ul>
      </div>
    </div>
  )
...
// Enhanced StudentsTab Component with Password Features
function StudentsTab({ students, onRefresh, user }) {
  const [searchTerm, setSearchTerm] = useState('')
  const [filterStatus, setFilterStatus] = useState('all')
  const [loading, setLoading] = useState(false)
  ...
  const resetForm = () => {
    ...
    setPasswordVisibility({ add: false, edit: false })
  }
  ...
  const handleAddStudent = async (e) => {
    ...
    try {
      const response = await fetch('/api/students', {
        ...
        body: JSON.stringify({
          ...studentForm,
          school_id: user.school_id || user.SchoolID || 2,
          name: studentForm.name.trim()
        })
      })
  ...
  const handleEditStudent = async (e) => {
    ...
    try {
      ...
      if (result.success) {
        // If parent password was updated, also create/update parent record
        if (studentForm.parent_password) {
          await createOrUpdateParentLogin(editingStudent.id || editingStudent.student_id, studentForm.parent_password)
        }
  ...
  const createOrUpdateParentLogin = async (studentId, password) => {
    try {
      ...
      if (!result.success) {
        console.warn('Failed to create/update parent login:', result.error)
      }
    } catch (error) {
      console.error('Error creating/updating parent login:', error)
    }
  }
            ...
            <form onSubmit={handleAddStudent} className="space-y-4">
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    type={passwordVisibility.add ? "text" : "password"}
                    value={studentForm.parent_password}
                    onChange={(e) => setStudentForm({...studentForm, parent_password: e.target.value})}
          ...
          <div className="bg-white rounded-lg p-4 sm:p-6 w-full max-w-md mx-4">
            ...
            <form onSubmit={handleEditStudent} className="space-y-4">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500 mt-1">
                  {(editingStudent.parentPasswordSet || editingStudent.parent_password_set) ? 
                    'Parent already has login. Enter new password to update.' : 
                    'No parent password set. Enter password to create parent login.'
                  }
                </p>
              </div>
