...
export default function CustomSchoolAdminDashboard({ user, onLogout }: CustomSchoolAdminDashboardProps) {
  ...
  const loadDashboardData = async () => {
    ...
    try {
      ...
      setStats({
        ...
        sync_last_heartbeat: overviewData?.sync_last_heartbeat
      });
    } catch (error) {
      console.error('Error loading dashboard data:', error);
    } finally {
  ...
  const secondaryColor = user?.theme?.secondary || '#10b981';
  const logoUrl = user?.theme?.logo;
  const schoolName = user.school?.name || 'School Administration';
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-2xl font-bold text-gray-900">{schoolName}</h1>
                <p className="text-sm text-indigo-600 font-medium">Admin Dashboard</p>
              </div>
            </div>
            <div className="flex items-center gap-4">
              <div className="text-right hidden md:block">
                <p className="text-sm font-semibold text-gray-900">{user.username || 'Admin'}</p>
                <p className="text-xs text-indigo-600 capitalize">{user.role?.replace('_', ' ')}</p>
              </div>
              <Button 
                variant="outline" 
                onClick={onLogout} 
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          <Card 
            className="p-6 border-2 shadow-lg hover:shadow-xl transition-shadow"
            style={{ 
          ...
          >
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-bold mb-1" style={{ color: primaryColor }}>
                  Total Students
                </p>
                <p className="text-4xl font-extrabold" style={{ color: primaryColor }}>
                  {stats.total_students}
                </p>
              </div>
              <div 
          ...
          <Card 
            className="p-6 border-2 shadow-lg hover:shadow-xl transition-shadow cursor-pointer"
            onClick={() => setActiveTab('attendance')}
            style={{
              backgroundColor: 'white',
          ...
          >
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-bold mb-1" style={{ color: secondaryColor }}>Present Today</p>
                <p className="text-4xl font-extrabold" style={{ color: secondaryColor }}>{stats.present_today}</p>
                <p className="text-xs mt-1 font-medium" style={{ color: secondaryColor }}>Click to view →</p>
              </div>
              <div 
                className="w-16 h-16 rounded-2xl flex items-center justify-center"
                style={{ backgroundColor: `${secondaryColor}15` }}
              >
                <ClipboardCheck className="h-8 w-8" style={{ color: secondaryColor }} />
              </div>
            </div>
          </Card>
          
          <Card 
            className="p-6 bg-white border-2 border-red-300 shadow-lg hover:shadow-xl transition-shadow cursor-pointer"
            onClick={() => setActiveTab('attendance')}
          >
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-red-700 font-bold mb-1">Absent Today</p>
                <p className="text-4xl font-extrabold text-red-700">{stats.absent_today}</p>
                <p className="text-xs text-red-600 mt-1 font-medium">Click to view →</p>
              </div>
          ...
          <Card className="p-6 bg-white border-2 border-amber-300 shadow-lg hover:shadow-xl transition-shadow">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-amber-700 font-bold mb-1">Need Setup</p>
                <p className="text-4xl font-extrabold text-amber-700">{stats.students_without_passwords}</p>
              </div>
        ...
        <Card className="mb-8 shadow-lg bg-white">
          <div className="hidden md:block border-b border-indigo-100">
            <nav className="flex space-x-8 px-6">
              {tabs.map(tab => (
                <button 
                  key={tab.id}
          ...
          <div className="md:hidden border-b border-indigo-100">
            <div className="px-4 py-3">
              <button
                onClick={() => setShowMobileMenu(!showMobileMenu)}
                className="flex items-center justify-between w-full"
                ...
                <div className="mt-3 space-y-1">
                  {tabs.map(tab => (
                    <button
                      ...
                      className="flex items-center w-full text-left px-3 py-2 rounded-md text-sm font-semibold text-gray-700 hover:bg-indigo-50"
                    >
                      <span className="text-lg mr-3">{tab.icon}</span>
                      {tab.label}
                    </button>
